// ; (function ($, window) {
//     "use strict"; var guid = 0, userAgent = (window.navigator.userAgent || window.navigator.vendor || window.opera), isFirefox = /Firefox/i.test(userAgent), isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry/i.test(userAgent), isFirefoxMobile = (isFirefox && isMobile), $body = null; var options = { callback: $.noop, cover: false, customClass: "", label: "", external: false, links: false, mobile: false, trim: 0 }; var pub = {
//         defaults: function (opts) { options = $.extend(options, opts || {}); return (typeof this === 'object') ? $(this) : true; }, disable: function (option) {
//             return $(this).each(function (i, input) {
//                 var data = $(input).parent(".selecter").data("selecter"); if (data) {
//                     if (typeof option !== "undefined") { var index = data.$items.index(data.$items.filter("[data-value=" + option + "]")); data.$items.eq(index).addClass("disabled"); data.$options.eq(index).prop("disabled", true); } else {
//                         if (data.$selecter.hasClass("open")) { data.$selecter.find(".selecter-selected").trigger("click.selecter"); }
//                         data.$selecter.addClass("disabled"); data.$select.prop("disabled", true);
//                     }
//                 }
//             });
//         }, destroy: function () {
...
//                     data.$select[0].tabIndex = data.tabIndex; data.$select.find(".selecter-placeholder").remove(); data.$selected.remove(); data.$itemsWrapper.remove(); data.$selecter.off(".selecter"); data.$select.off(".selecter").removeClass("selecter-element").show().unwrap();
//                 }
//             });
//         }, enable: function (option) { return $(this).each(function (i, input) { var data = $(input).parent(".selecter").data("selecter"); if (data) { if (typeof option !== "undefined") { var index = data.$items.index(data.$items.filter("[data-value=" + option + "]")); data.$items.eq(index).removeClass("disabled"); data.$options.eq(index).prop("disabled", false); } else { data.$selecter.removeClass("disabled"); data.$select.prop("disabled", false); } } }); }, refresh: function () { return pub.update.apply($(this)); }, update: function () { return $(this).each(function (i, input) { var data = $(input).parent(".selecter").data("selecter"); if (data) { var index = data.index; data.$allOptions = data.$select.find("option, optgroup"); data.$options = data.$allOptions.filter("option"); data.index = -1; index = data.$options.index(data.$options.filter(":selected")); _buildOptions(data); if (!data.multiple) { _update(index, data); } } }); }
//     }; function _init(opts) {
//         opts = $.extend({}, options, opts || {}); if ($body === null) { $body = $("body"); }
//         var $items = $(this); for (var i = 0, count = $items.length; i < count; i++) { _build($items.eq(i), opts); }
//         return $items;
//     }
//     function _build($select, opts) {
//         if (!$select.hasClass("selecter-element")) {
//             opts = $.extend({}, opts, $select.data("selecter-options")); opts.multiple = $select.prop("multiple"); opts.disabled = $select.is(":disabled"); if (opts.external) { opts.links = true; }
...
//         var html = '', itemTag = (data.links) ? "a" : "span", j = 0; for (var i = 0, count = data.$allOptions.length; i < count; i++) {
//             var $op = data.$allOptions.eq(i); if ($op[0].tagName === "OPTGROUP") {
//                 html += '<span class="selecter-group'; if ($op.is(":disabled")) { html += ' disabled'; }
//                 html += '">' + $op.attr("label") + '</span>';
//             } else {
//                 var opVal = $op.val(); if (!$op.attr("value")) { $op.attr("value", opVal); }
...
//     function _onOpen(e) {
//         e.preventDefault(); e.stopPropagation(); var data = e.data; if (!data.$selecter.hasClass("open")) {
//             var offset = data.$selecter.offset(), bodyHeight = $body.outerHeight(), optionsHeight = data.$itemsWrapper.outerHeight(true), selectedOffset = (data.index >= 0) ? data.$items.eq(data.index).position() : { left: 0, top: 0 }; if (offset.top + optionsHeight > bodyHeight) { data.$selecter.addClass("bottom"); }
...
//         e.preventDefault(); e.stopPropagation(); var $target = $(this), data = e.data; if (!data.$select.is(":disabled")) {
//             if (data.$itemsWrapper.is(":visible")) { var index = data.$items.index($target); if (index !== data.index) { _update(index, data); _handleChange(data); } }
...
//         var data = e.data; if (e.keyCode === 13) {
//             if (data.$selecter.hasClass("open")) { _onClose(e); _update(data.index, data); }
//             _handleChange(data);
//         } else if (e.keyCode !== 9 && (!e.metaKey && !e.altKey && !e.ctrlKey && !e.shiftKey)) {
//             e.preventDefault(); e.stopPropagation(); var total = data.$items.length - 1, index = (data.index < 0) ? 0 : data.index; if ($.inArray(e.keyCode, (isFirefox) ? [38, 40, 37, 39] : [38, 40]) > -1) {
//                 index = index + ((e.keyCode === 38 || (isFirefox && e.keyCode === 37)) ? -1 : 1); if (index < 0) { index = 0; }
//                 if (index > total) { index = total; }
//             } else {
//                 var input = String.fromCharCode(e.keyCode).toUpperCase(), letter, i; for (i = data.index + 1; i <= total; i++) { letter = data.$options.eq(i).text().charAt(0).toUpperCase(); if (letter === input) { index = i; break; } }
...
//         if (pub[method]) { return pub[method].apply(this, Array.prototype.slice.call(arguments, 1)); } else if (typeof method === 'object' || !method) { return _init.apply(this, arguments); }
//         return this;
//     }; $.selecter = function (method) { if (method === "defaults") { pub.defaults.apply(this, Array.prototype.slice.call(arguments, 1)); } };
// })(jQuery, window);
